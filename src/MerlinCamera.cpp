...
int Camera::readFrame(void *bptr, int frame_nb, double timeout_secs) {
	...
	std::cout << "Camera::readFrame() " << DEB_VAR5(frame_nb, m_image_type, m_npixels, m_nrasters, m_counter) << std::endl;
	int selectStatus = m_merlin->select(m_pipes[0], tv);
	AutoMutex aLock(m_cond.mutex());
	std::cout << "Camera::readFrame() " << DEB_VAR1(selectStatus) << std::endl;
	switch (selectStatus)
	{
...
//	AutoMutex aLock(m_cam.m_cond.mutex());
	...
	while (!m_cam.m_quit) {
		while (m_cam.m_wait_flag && !m_cam.m_quit) {
			std::cout << "Wait for start acquisition" << std::endl;
		...
		if (m_cam.m_quit)
			return;
		std::cout << "AcqThread Running" << std::endl;
		...
		while (continueFlag && (!m_cam.m_nb_frames || m_cam.m_acq_frame_nb < m_cam.m_nb_frames)) {
			...
			std::cout << "before getFrameBufferptr" << std::endl;
			...
			case Camera::STOP_ISSUED:
				...
				timeout = 0.25;
				std::cout << "Stop called, timeout modified to 0.25 secs" << std::endl;
				break;
			...
			case Camera::STOPPED_AND_TIMEOUT:
				continueFlag = false;
				std::cout << "Stopped with read timeout" << std::endl;
				break;
			case Camera::READ_OK:
				frame_info.acq_frame_nb = m_cam.m_acq_frame_nb;
				if ((continueFlag = buffer_mgr.newFrameReady(frame_info))) {
					std::cout << "acqThread::threadFunction() newframe ready " << std::endl;
					AutoMutex lock(m_cam.m_cond.mutex());
					++m_cam.m_acq_frame_nb;
					std::cout << "acquired " << m_cam.m_acq_frame_nb << " frames, required " << m_cam.m_nb_frames << " frames" << std::endl;
				}
				break;
			default:
			  std::cout << " AcqThread::threadfunction() This should not happen: " << DEB_VAR1(rc) << std::endl;
			}
		}
		auto t2 = Clock::now();
		std::cout << "Delta t2-t1: " << std::chrono::duration_cast < std::chrono::nanoseconds
				> (t2 - t1).count() << " nanoseconds" << std::endl;
		std::cout << " AcqThread::threadfunction() Setting thread running flag to false" << std::endl;
		AutoMutex lock(m_cam.m_cond.mutex());
//		aLock.lock();
		m_cam.m_thread_running = false;
...
void Camera::getDetectorStatus(DetectorStatus &status) {
	...
	std::cout << "Detector status returned: " << stat << std::endl;
